...
public class BluetoothLeService extends Service {
	...
	public static String BELT_DEVICE_SERVICE = "0000fff0-0000-1000-8000-00805f9b34fb";

	public static String BELT_BLOOD_GATT_CHARACTERISTICS = "0000fff4-0000-1000-8000-00805f9b34fb";
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
			...
			if (newState == BluetoothProfile.STATE_CONNECTED) {
				...
				mEventBus.post(new BlueToothEvent.GattConnectedEvent());
				...
				mEventBus.post(new BlueToothEvent.GattDisConnectedEvent());
		...
		public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				processGattCharacteristic(characteristic);
			}
		}
	...
	/**
	 * @description：onCharacteristicRead和onCharacteristicChanged触发
	 * @author samy
	 * @date 2015-2-24 下午9:55:38
	 */
	private void processGattCharacteristic(BluetoothGattCharacteristic bluetoothGattCharacteristic) {
		if (bluetoothGattCharacteristic != null) {
			final byte[] data = bluetoothGattCharacteristic.getValue();

			int[] result = new int[data.length];
			for (int i = 0; i < data.length; i++) {
				result[i] = ByteHexHelper.byteToInt(data[i]);
			}
			DataSysEvent event = new DataSysEvent();
			event.currentBloodPressure = result[1];
			mEventBus.post(event);
			if (data.length > 10) {
				ResultEvent eventResult = new ResultEvent();
				eventResult.systolic = data[2]; // 收缩压
				eventResult.diastolic = data[4]; // 舒张压
				eventResult.pulse = data[8]; // 脉搏
				mEventBus.post(eventResult);
			}
	...
	public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic, boolean enabled) {
		...
		// Log.e("zz", "descriptor===>" + descriptor.getUuid());
		// }
