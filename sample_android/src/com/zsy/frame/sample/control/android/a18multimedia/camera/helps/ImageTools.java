...
public class ImageTools {
		...
		try {
			...
			try {
				...
				if (opts.inSampleSize > 1 && opts.inSampleSize <= 4) {// 测试结果
					...
					Log.i("samy", srcSize + "压缩rate=" + rate);
	 ...
	 * @return
	 */
	public static File getShowImageFile(String tempFilePath) {
		try {
			Bitmap temp = getShowImage(tempFilePath);
			File resFile = initTempFile();
			temp.compress(CompressFormat.JPEG, 100, new FileOutputStream(resFile));
			temp.recycle();
			return resFile;
	...
	public static Intent getTakeCameraIntent(Uri photoUri) {
		...
		openCameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, photoUri);// 如果传递这个参数，那么拍到的相片会存一份到uri指向的路径
	...
	/**
	 * 图库选择图片、剪裁图片并压缩，和拍照回调可做整合 一般用于获取大图；Uri方式 无法加载预览图片；
	 */
	...
	public static File initTempFile() {
		File uploadFileDir = new File(TakePhotoBySurfaceviewAct.imgPath);
	...
	public static String getRealFilePath(Context context) {
		...
		Cursor cursor = context.getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, null, null, null, Media.DATE_MODIFIED + " desc");
