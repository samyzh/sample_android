...
public class DefaultHeaderTransformer extends HeaderTransformer {
	...
	protected DefaultHeaderTransformer() {
		...
		if (Build.VERSION.SDK_INT < min) { throw new IllegalStateException("This HeaderTransformer is designed to run on SDK " + min + "+. If using ActionBarSherlock or ActionBarCompat you should use the appropriate provided extra."); }
	}
	...
	private void setupViewsFromStyles(Activity activity, View headerView) {
		...
		if (styleAttrs.hasValue(R.styleable.PullToRefreshHeader_ptrSmoothProgressBarStyle)) {
			...
			if (spbStyleRes != 0) mHeaderProgressBar.applyStyle(spbStyleRes);
