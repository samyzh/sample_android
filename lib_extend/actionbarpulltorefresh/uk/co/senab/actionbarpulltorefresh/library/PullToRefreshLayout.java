...
public class PullToRefreshLayout extends FrameLayout {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (isEnabled() && mPullToRefreshAttacher != null && getChildCount() > 0) { return mPullToRefreshAttacher.onInterceptTouchEvent(event); }
		return false;
	}
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		if (isEnabled() && mPullToRefreshAttacher != null) { return mPullToRefreshAttacher.onTouchEvent(event); }
	...
	private void ensureAttacher() {
		if (mPullToRefreshAttacher == null) { throw new IllegalStateException("You need to setup the PullToRefreshLayout before using it"); }
	}
	...
	static class LayoutParams extends FrameLayout.LayoutParams {
		...
		LayoutParams(Context c, AttributeSet attrs) {
			...
			mViewDelegateClassName = a.getString(R.styleable.PullToRefreshView_ptrViewDelegateClass);
			a.recycle();
		}
